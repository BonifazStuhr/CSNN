...
from Experiment_Component.ModelSuits.ATfRecordModelSuit import ATfRecordModelSuit

class StandardTfRecordModelSuit(ATfRecordModelSuit):
    """
    ...
    def __init__(self, model, dataset_provider, tfrecord_inputpipeline_config, batch_size, trainer, model_dir="/model",
        ...
        """
        super().__init__(model, dataset_provider, tfrecord_inputpipeline_config, batch_size, trainer,
                         model_dir=model_dir, save_checkpoint_interval=save_checkpoint_interval,
    ...
    def doTraining(self, train_steps, eval_interval, only_save_best_checkpoints=True):
        ...
        self._logger.train(
            ...
            "StandardTfRecordModelSuit:doTraining")
        ...
        try:
            ...
            self._sess.run(self._iterator_train.initializer,
                            feed_dict={self._file_placeholder_train: self._dataset_files["train"]})
            ...
            while self._global_step < train_steps:
                ...
                if self._global_step % self._log_interval == 0:
                    ...
                    self._logger.train("Step " + str(self._global_step) + ": " + str(self._log_interval) +
                                       ...
                                       "StandardTfRecordModelSuit:doTraining", self._global_step, train_steps)
                    start_log_loss_steps_time = time.time()
            ...
            self._logger.train("Finished training for " + str(train_steps) + " steps. Evaluation was every " +
                               ...
                               "%. Final loss value: " + str(loss_value), "StandardTfRecordModelSuit:doTraining")

            self.doValidation("test")

        except tf.errors.OutOfRangeError:
            self._logger.warning(
                "Training iterator went out of range in step: " + str(self._global_step)+
                " before the training finished!", "StandardTfRecordModelSuit:doTraining")
    ...
    def doValidation(self, mode):
        ...
        """
        self._logger.val("Started validation for " + str(mode) + "...", "StandardTfRecordModelSuit:doValidation")
        ...
        try:
            ...
            self._logger.val(
                ...
                "StandardTfRecordModelSuit:__doValidation")
                ...
                while True:
                    ...
                    if (inf_step % self._log_interval == 0):
                        ...
                        self._logger.val("Step " + str(self._global_step) + ", " + str(mode) + " step " + str(inf_step)
                                 + ": " + str(self._log_interval) + " " + str(mode) + " steps past in " +
                                str(end_log_loss_steps_time - start_log_loss_steps_time),
                                         "StandardTfRecordModelSuit:infer")
                        start_log_loss_steps_time = time.time()
